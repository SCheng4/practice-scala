...
object ListManipulationExercise01 {
   ...
   */
  def sumOfList(l: List[Int]): Int = if (l.isEmpty) 0 else l.head + sumOfList(l.tail)
   ...
   *  - ... etc
   */
  def lastElementInList[T](l: List[T]): T = l.reverse.head
   ...
   *  - ... etc 
   */
  def concatLists[T](l1: List[T], l2: List[T]): List[T] = List.concat(l1, l2)
   ...
   */
  def elementExists[T](l: List[T], e: T): Boolean = if (l.isEmpty) false
  													else if (l.head == e) true
  													else elementExists(l.tail, e) 
